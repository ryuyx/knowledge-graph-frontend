                ...
                {nodeDetail ? (
                    nodeDetail.error ? (
                        ...
                        <div className="text-left break-all whitespace-pre-wrap h-[90%] overflow-auto">
                            ...
                            <div className="mb-2 text-xs text-gray-400">Type: {nodeDetail.source_type}</div>
                            <div className="mb-2 text-xs text-gray-400">Status: {nodeDetail.status}</div>
                            <div className="mb-2 text-xs text-gray-400">Created at: {nodeDetail.created_at}</div>
                            <div className="mb-2 text-xs text-gray-400">Filename: {nodeDetail.metadata?.original_filename}</div>
                            <div className="mb-2 text-xs text-gray-400">File size: {nodeDetail.metadata?.file_size} bytes</div>
                            <div className="mb-2 text-xs text-gray-400">Content type: {nodeDetail.metadata?.content_type}</div>
                        ...
                        <div className="text-left break-all h-[90%] overflow-auto">
                            <div className="mb-2 font-semibold text-xl">Category: {nodeDetail.big_hot_word_name}</div>
                            <div className="mb-2 text-xs text-gray-400">ID: {nodeDetail.big_hot_word_id}</div>
                            <div className="mb-2 text-xs text-gray-400">Hot words count: {nodeDetail.hot_words.length}</div>
                            <div className="mt-4">
                                <div className="space-y-4">
                                    {nodeDetail.hot_words.map((hotWord: any) => (
                                        <div key={hotWord.hot_word_id} className="border border-base-300 rounded-lg p-3">
                                            <div className="font-semibold mb-2">Topic: {hotWord.hot_word_name}</div>
                                            <div className="text-xs text-gray-400 mb-2">ID: {hotWord.hot_word_id}</div>
                                            <div className="text-xs text-gray-400 mb-2">Knowledge item count: {hotWord.total_count}</div>
                        ...
                        <div className="text-left break-all h-[90%] overflow-auto">
                            <div className="mb-2 font-semibold text-xl">Topic: {nodeDetail.hot_word_name}</div>
                            <div className="mb-2 text-xs text-gray-400">ID: {nodeDetail.hot_word_id}</div>
                            <div className="mb-2 text-xs text-gray-400">Knowledge item count: {nodeDetail.total_count}</div>
                            <div className="mt-4">
                                <table className="table w-full text-xs">
                                    <thead>
                                            <tr className="bg-base-200">
                        ...
                        <div className="text-left break-all">
                            {nodeDetail.title || nodeDetail.name || 'No details available'}
                        </div>
                    )
                ) : (
